 ...
 */
contract FollowNFT is PoPPNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        //return IPoPPHub(HUB).getFollowNFTURI(_profileId);
        string memory handle = IPoPPHub(HUB).getHandle(_profileId);
        string memory imageURI = IPoPPHub(HUB).imageURI(_profileId);
        return
        string(
            abi.encodePacked(
                ...
                Base64.encode(
                    abi.encodePacked(
                        '{"name":"',
                        handle,
                        '-Soul Bound","description":"',
                        handle,
                        '\'s soul binder","image":"',
                        ProfileTokenURILogic.getSVGImageBase64Encoded(handle, imageURI),
                        '"}'
                    )
                )
            )
    ...
    function _beforeTokenTransfer(
        ...
        address followModule = IPoPPHub(HUB).getFollowModule(_profileId);

        _moveDelegate(fromDelegatee, toDelegatee, 1);

        super._beforeTokenTransfer(from, to, tokenId);
        IPoPPHub(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);
