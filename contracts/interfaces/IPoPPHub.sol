 ...
 *
 * @notice This is the interface for the PoPPHub contract, the main entry point for the PoPP Protocol.
 * You'll find all the events and external functions, as well as the reasoning behind them here.
 */
interface IPoPPHub {
    /**
     * @notice Initializes the PoPPHub NFT, setting the initial governance address as well as the name and symbol in
     * the PoPPNFTBase contract.
     *
    ...
    function whitelistFinanceModule(address followModule, bool whitelist) external;
     ...
     *
     * NOTE: Both the `profileId` and `data` arrays must be of the same length, regardless if the profiles do not have a follow module set.
     *
     * @param profileId The token ID array of the profiles to follow.
     * @param data The arbitrary data array to pass to the follow module for each profile if needed.
     *
    ...
    function isFinanceModuleWhitelisted(address financeModule) external view returns (bool);
     ...
     *
     * @return address The tokenId of the collectNFT associated with the queried publication.
     */
    function getCollectNFT(uint256 profileId, uint256 pubId) external view returns (uint256);
    function getMirrorNFT(uint256 profileId, uint256 pubId) external view returns (uint256);
    ...
    function getFinancePoolImpl() external view returns (address);
     ...
     */
    function getEchoNFTImpl() external view returns (address);
    function getMirrorNFTImpl() external view returns (address);
